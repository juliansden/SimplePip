defmodule LogTest do
  ...
  test "update_commit_index works" do
    config = Raft.new_configuration([:a, :b, :c, :d, :e], :a, 100, 1000, 20)
    match_index = %{a: 0, b: 5, c: 4, d: 8, e: 9}
    config = %{config | commit_index: 1, match_index: match_index}
    assert Raft.update_commit_index(config).commit_index == 8
    match_index = %{a: 0, b: 3, c: 4, d: 1, e: 2}
    config = %{config | commit_index: 1, match_index: match_index}
    assert Raft.update_commit_index(config).commit_index == 3
  end
end
